export default async function handler(req, res) {
  ...
  if (!HF_TOKEN) return res.status(500).json({ error: 'HF_TOKEN set nahi hai' });
  ...
  try {
    ...
    if (!hfRes.ok) {
      ...
      if (hfRes.status === 503) msg = 'Model busy hai — thodi der baad try karo';
      if (hfRes.status === 401) msg = 'HF Token galat hai';
